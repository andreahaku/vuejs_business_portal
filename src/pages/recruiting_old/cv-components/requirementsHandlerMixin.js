// id dropdowns always like #dropdownMenuReq + index
// vals in skill.itemVals and boolean LoadingLiv
export const requirementsHandlerMixin = {
    data: function () {
        return {
            ...
            vals: [{ val: 1, desc: 'Basso' },
            { val: 2, desc: 'Medio-Basso' },
            { val: 3, desc: 'Medio' },
            { val: 4, desc: 'Medio-Alto' },
            { val: 5, desc: 'Alto' }
            ]
        }
    },
                ...
                this.$http.get(localStorage.getItem("rec-path") + "/Livelli?IDRequisiti=" + id, {
                    ...
                    if (res && res.Livelli) {
                        ...
                        for (var i = 0; i < res.Livelli.length; i++) {
                            newVals.push({ val: i + 1, desc: res.Livelli[i].DescrizioneLivello });
                        }
                        this.skills[ind].itemVals = newVals;
                        this.$store.commit('addReqLevelMapping', { id: id, levels: newVals });
                    } else {
                        this.skills[ind].itemVals = this.vals;
                        this.$store.commit('addReqLevelMapping', { id: id, levels: this.vals });
                    }
                    this.skills[ind].loadingLiv = false;
                    this.forceDOM();
                }, function (err) {
