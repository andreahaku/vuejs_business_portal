...
export default {
    ...
    methods: {
        ...
        findCV() {
            ...
            //se ancora l'anagrafica non è presente
            if (a == null || a == {}) {
                // MOD -> http get con dati attuali
                this.$http.get(localStorage.getItem("rec-path") + "/PersonaCV?ID=" + localStorage.getItem("idpersona"), {
                    headers: {
                        'child-level': "9999",
                    ...
                    if (res && res.PersonaCV && res.PersonaCV[0]) {
                        if (res.PersonaCV[0].Competenza == null &&
                            ...
                            //se esiste un cv ma nessuna tab è stata inserita: conservo info utili per il wizard
                            //e avvio il wizard segnalando cv detected false
                            ...
                            //commit informazioni disponibili
                            this.commitData(res.PersonaCV[0]);
                            //con tutte le informazini disponibili sul vue store
                            this.cvDetected = 1;
                            //abilito visualizzazione cv
                            this.loading = false;
                        }
                    }
                }, function(err) {
                ...
                //con anagrafica presente, si suppone di avere già le informazioni disponibili (da accertare)
                this.cvDetected = 1;
                this.loading = false;
            }
        ...
        commitData(cv) {
            ...
            if (cv.EsperienzeProfessionali && cv.EsperienzeProfessionali.length > 0) {
                ...
                for (var i = 0; i < cv.EsperienzeProfessionali.length; i++) {
                    ...
                    //NON SI PUO' VEDERE -- DA FIXARE E TESTARE 
                    if (obj.da.indexOf("-") != -1) {
                        var da = obj.da.split("-")
                        da.reverse();
                        var n = da[0];
        ...
        // ! CHECK TOKEN NON VIENE PIU' INVOCATA
        checkToken() {
            ...
            if (localStorage.getItem("career-user-logged") == null) {
                ...
                this.$http.get(localStorage.getItem("rec-path") + "/Webapi", {
                    ...
                    if (response.body != null) {
                        ...
                        this.baseToken.token = "recruiting_token";
                        this.findCV();
                    } else {
                ...
                //il token non dovrebbe più servire
                this.baseToken.token = "recruiting_token";
                this.findCV();
            }
        },
