        ...
        <div class="media-body">
             <div class="media v-middle">
               <div class="media-right">
                 <h4 class="text-headline margin-none">Capacità e competenze
                 </h4>
               </div>
             </div>
                            ...
                            <form class="form-horizontal row" v-for="(skill, index) in skills" :key="index">
                                ...
                                <div v-if="gotReqs" class="form-group col-xs-12">
                                     ...
                                     <div class="col-xs-11" style="padding-right: 0px;">
                                         ...
                                         v-model="skill.required.nome" @keydown="filterReqs(index, skill.required.nome)"
                                         :class="{'missing': skill.missingFields.nome}" @blur="blur(index)" @focus="focus(index)">
                                     </div>
                                     <div class="col-xs-1" style="padding-left: 0px;">
                                          <button class="btn btn-default" type="button" data-toggle="collapse" :href="'#dropdownMenuReq'+index" aria-expanded="false"
                                          :aria-controls="'dropdownMenuReq'+index" 
                                          style="background-color: #eeeeee; height: 36px; padding-top: 14px;" @click="focus(index)">
                                            <span class="caret" style="position: relative; top: -5px;"></span>
                                          </button>
                                      </div>
                                      <div class="col-xs-12 collapse" :id="'dropdownMenuReq'+index" style="
                                        position: absolute;
                                        top: 60px;
                                        z-index: 2; 
                                        max-height: 240px;
                                        ...
                                        padding-right: 0px;">
                                         <div class="panel panel-default" style="padding: 5px 5px;">
                                              <div v-for="req in reqs" class="panel-body item-req" style="padding: 3px 5px;" @click="thisReq(index, req.ID, req.nome)">{{req.nome}}</div>   
                                         </div>
                                      </div>
                                </div>

                                <div class="form-group col-xs-12">
                                  <label for="end" class="col-sm-12 control-label"><strong>* </strong>Valutazione</label>
                                  
                                    
                                      <form style="padding: 5px;">
                                         <div class="col-xs-12 col-sm-10" style="margin-top: 7px;"
                                            :class="{'missing': skill.missingFields.livello}">
                                             <input :id="'rangeIE'+index" type="range" min='1' :max="skill.itemVals.length"
                                             v-model="skill.required.livello" @mouseup="leaveIE(index)"> 
                                         </div>
                                         <div class="col-xs-12 col-sm-2" style="margin-top: 5px;">{{skill.itemVals[skill.required.livello-1].desc}}</div>
                                         <div class="col-xs-12 col-lg-2" style="position: relative;">
                                            <div v-if="skill.loadingLiv" class='uil-rolling-css' style='transform:scale(0.12); 
                                            position: absolute; top: -100px; right: 0px; '><div><div></div><div></div></div></div>
                                         </div>
                                      </form>
                                </div>
                                ...
                                <div class="form-group col-xs-12">
                                  ...
                                  <div class="col-sm-12">
                                    <textarea rows="2" cols="50" class="form-control checkableInput" placeholder="Note .." style="margin-left: 0px; padding-left: 10px;" v-model="skill.required.note"></textarea>
                                  </div>
                                </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            leaveIE(i){
                if(this.isIE){
                    this.skills[i].required.livello = document.getElementById('rangeIE'+i).value;     
                }
            },
            checkRequired(){
                this.valid = 1;
                ...
                for(var i = 0; i < this.skills.length; i++){
                    ...
                    if(this.skills[i].required.nome.trim()===''
